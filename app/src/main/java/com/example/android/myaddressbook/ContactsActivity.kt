...
//        val notEmpty: (TextView) -> Boolean = { it.text.isNotEmpty() }
        val firstNameValid: (EditText) -> Boolean = { mFirstNameEdit.text.isNotEmpty() }
        val lastNameValid: (EditText) -> Boolean = { mLastNameEdit.text.isNotEmpty() }
        val emailValid: (EditText) -> Boolean = { Patterns.EMAIL_ADDRESS.matcher(mEmailEdit.text).matches() }
